...
class DeepSort(object):
    ...
    def update(self, bbox_xywh, confidences, cls, ori_img):
        self.height, self.width = ori_img.shape[:2]
        # generate detections
        features = self._get_features(bbox_xywh, ori_img)
        bbox_tlwh = self._xywh_to_tlwh(bbox_xywh)
        detections = [Detection(bbox_tlwh[i], conf, features[i], int(cls[i])) for i,conf in enumerate(confidences) if conf>self.min_confidence]
        ...
        for track in self.tracker.tracks:
            ...
            outputs.append(np.array([x1,y1,x2,y2,track_id,track.cls], dtype=float))
        if len(outputs) > 0:
